<template>
  <div class="justify-center my-custom-team-container items-center h-[900px] bg-gray-50">
    <div class="container team-container bg-white px-8 py-4 space-y-16 rounded-xl shadow h-[700px]">
      <h2 class="title text font-heading font-bold text-black text-4xl text-center">
        Meet the team
      </h2>

      <div
        class="card-embuh grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-y-16 gap-x-8 justify-items-center"
      >
        <div class="flex flex-col items-center text-center space-y-4">
          <img
            src="/public/img/aziz.jpeg"
            class="mx-auto h-40 w-40 rounded-full xl:w-56 xl:h-56 object-cover"
          />
          <h3 class="text-embuh font-medium text-black text-2xl">Riyanda Azizi</h3>
          <div class="logo-embuh flex space-x-4 justify-center items-center">
            <a href="#" target="_blank" class="social-icon-wrapper">
              <span class="sr-only">GitHub</span>
              <svg class="w-12 h-12" fill="currentColor" viewBox="0 0 29 29">
                <path
                  fill-rule="evenodd"
                  d="M1372.32,16.8097415 C1372.32,23.1517351 1376.33105,28.5314586 1381.89427,30.4295626 C1382.59472,30.5617425 1382.84997,30.1184991 1382.84997,29.7378209 C1382.84997,29.3976778 1382.83794,28.4944483 1382.83107,27.296898 C1378.9369,28.1639984 1378.11527,25.3723581 1378.11527,25.3723581 C1377.47841,23.7139404 1376.56052,23.2724594 1376.56052,23.2724594 C1375.2894,22.3824478 1376.65678,22.4000718 1376.65678,22.4000718 C1378.06198,22.5014098 1378.80111,23.8796059 1378.80111,23.8796059 C1380.04989,26.0729117 1382.07819,25.4393292 1382.87576,25.071869 C1383.00296,24.144847 1383.36478,23.5121457 1383.76443,23.1534975 C1380.6558,22.7913244 1377.38731,21.5594074 1377.38731,16.0589595 C1377.38731,14.4921866 1377.93306,13.2100411 1378.82861,12.207236 C1378.68422,11.8441818 1378.20379,10.384034 1378.96612,8.40838451 C1378.96612,8.40838451 1380.14099,8.02241909 1382.8156,9.87998785 C1383.93202,9.56099359 1385.13009,9.40237767 1386.32043,9.39620927 C1387.50991,9.40237767 1388.70712,9.56099359 1389.82526,9.87998785 C1392.49815,8.02241909 1393.6713,8.40838451 1393.6713,8.40838451 C1394.43535,10.384034 1393.95492,11.8441818 1393.81139,12.207236 C1394.70866,13.2100411 1395.25011,14.4921866 1395.25011,16.0589595 C1395.25011,21.5735066 1391.97647,22.7869184 1388.85838,23.1420419 C1389.3603,23.5852853 1389.80808,24.4611977 1389.80808,25.8006211 C1389.80808,27.7189926 1389.79089,29.2672603 1389.79089,29.7378209 C1389.79089,30.1220239 1390.04356,30.5687921 1390.75347,30.4286814 C1396.31239,28.5261714 1400.32,23.1499727 1400.32,16.8097415 C1400.32,8.8815887 1394.05118,2.455 1386.31871,2.455 C1378.58882,2.455 1372.32,8.8815887 1372.32,16.8097415 Z"
                  transform="translate(-1372 -2)"
                ></path>
              </svg>
            </a>
          </div>
        </div>

        <div class="flex flex-col items-center text-center space-y-4">
          <img
            src="/public/img/arvi.jpeg"
            class="mx-auto h-40 w-40 rounded-full xl:w-56 xl:h-56 object-cover"
          />
          <h3 class="text-embuh font-medium text-black text-2xl">Arvi Rizqi Fadhila</h3>
          <div class="logo-embuh flex space-x-4 justify-center items-center">
            <a href="#" target="_blank" class="social-icon-wrapper">
              <span class="sr-only">GitHub</span>
              <svg class="w-12 h-12" fill="currentColor" viewBox="0 0 29 29">
                <path
                  fill-rule="evenodd"
                  d="M1372.32,16.8097415 C1372.32,23.1517351 1376.33105,28.5314586 1381.89427,30.4295626 C1382.59472,30.5617425 1382.84997,30.1184991 1382.84997,29.7378209 C1382.84997,29.3976778 1382.83794,28.4944483 1382.83107,27.296898 C1378.9369,28.1639984 1378.11527,25.3723581 1378.11527,25.3723581 C1377.47841,23.7139404 1376.56052,23.2724594 1376.56052,23.2724594 C1375.2894,22.3824478 1376.65678,22.4000718 1376.65678,22.4000718 C1378.06198,22.5014098 1378.80111,23.8796059 1378.80111,23.8796059 C1380.04989,26.0729117 1382.07819,25.4393292 1382.87576,25.071869 C1383.00296,24.144847 1383.36478,23.5121457 1383.76443,23.1534975 C1380.6558,22.7913244 1377.38731,21.5594074 1377.38731,16.0589595 C1377.38731,14.4921866 1377.93306,13.2100411 1378.82861,12.207236 C1378.68422,11.8441818 1378.20379,10.384034 1378.96612,8.40838451 C1378.96612,8.40838451 1380.14099,8.02241909 1382.8156,9.87998785 C1383.93202,9.56099359 1385.13009,9.40237767 1386.32043,9.39620927 C1387.50991,9.40237767 1388.70712,9.56099359 1389.82526,9.87998785 C1392.49815,8.02241909 1393.6713,8.40838451 1393.6713,8.40838451 C1394.43535,10.384034 1393.95492,11.8441818 1393.81139,12.207236 C1394.70866,13.2100411 1395.25011,14.4921866 1395.25011,16.0589595 C1395.25011,21.5735066 1391.97647,22.7869184 1388.85838,23.1420419 C1389.3603,23.5852853 1389.80808,24.4611977 1389.80808,25.8006211 C1389.80808,27.7189926 1389.79089,29.2672603 1389.79089,29.7378209 C1389.79089,30.1220239 1390.04356,30.5687921 1390.75347,30.4286814 C1396.31239,28.5261714 1400.32,23.1499727 1400.32,16.8097415 C1400.32,8.8815887 1394.05118,2.455 1386.31871,2.455 C1378.58882,2.455 1372.32,8.8815887 1372.32,16.8097415 Z"
                  transform="translate(-1372 -2)"
                ></path>
              </svg>
            </a>
          </div>
        </div>

        <div class="flex flex-col items-center text-center space-y-4">
          <img
            src="/public/img/sindhu.jpeg"
            class="mx-auto h-40 w-40 rounded-full xl:w-56 xl:h-56 object-cover"
          />
          <h3 class="text-embuh font-medium text-black text-2xl">Yohanes Sindhunata</h3>
          <div class="logo-embuh flex space-x-4 justify-center items-center">
            <a href="#" target="_blank" class="social-icon-wrapper">
              <span class="sr-only">GitHub</span>
              <svg class="w-12 h-12" fill="currentColor" viewBox="0 0 29 29">
                <path
                  fill-rule="evenodd"
                  d="M1372.32,16.8097415 C1372.32,23.1517351 1376.33105,28.5314586 1381.89427,30.4295626 C1382.59472,30.5617425 1382.84997,30.1184991 1382.84997,29.7378209 C1382.84997,29.3976778 1382.83794,28.4944483 1382.83107,27.296898 C1378.9369,28.1639984 1378.11527,25.3723581 1378.11527,25.3723581 C1377.47841,23.7139404 1376.56052,23.2724594 1376.56052,23.2724594 C1375.2894,22.3824478 1376.65678,22.4000718 1376.65678,22.4000718 C1378.06198,22.5014098 1378.80111,23.8796059 1378.80111,23.8796059 C1380.04989,26.0729117 1382.07819,25.4393292 1382.87576,25.071869 C1383.00296,24.144847 1383.36478,23.5121457 1383.76443,23.1534975 C1380.6558,22.7913244 1377.38731,21.5594074 1377.38731,16.0589595 C1377.38731,14.4921866 1377.93306,13.2100411 1378.82861,12.207236 C1378.68422,11.8441818 1378.20379,10.384034 1378.96612,8.40838451 C1378.96612,8.40838451 1380.14099,8.02241909 1382.8156,9.87998785 C1383.93202,9.56099359 1385.13009,9.40237767 1386.32043,9.39620927 C1387.50991,9.40237767 1388.70712,9.56099359 1389.82526,9.87998785 C1392.49815,8.02241909 1393.6713,8.40838451 1393.6713,8.40838451 C1394.43535,10.384034 1393.95492,11.8441818 1393.81139,12.207236 C1394.70866,13.2100411 1395.25011,14.4921866 1395.25011,16.0589595 C1395.25011,21.5735066 1391.97647,22.7869184 1388.85838,23.1420419 C1389.3603,23.5852853 1389.80808,24.4611977 1389.80808,25.8006211 C1389.80808,27.7189926 1389.79089,29.2672603 1389.79089,29.7378209 C1389.79089,30.1220239 1390.04356,30.5687921 1390.75347,30.4286814 C1396.31239,28.5261714 1400.32,23.1499727 1400.32,16.8097415 C1400.32,8.8815887 1394.05118,2.455 1386.31871,2.455 C1378.58882,2.455 1372.32,8.8815887 1372.32,16.8097415 Z"
                  transform="translate(-1372 -2)"
                ></path>
              </svg>
            </a>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<style scoped>
.card-embuh {
  margin-top: 100px;
}

.text-embuh {
  font-size: 1.25rem;
  margin-top: 50px;
}

.logo-embuh {
  margin-top: 10px;
}

.title {
  font-weight: 600;
  font-size: 3rem;
  color: var(--font-black);
}

.my-custom-team-container {
  width: 100%;
  margin: 0 auto;
  padding: 100px;
}

.team-container {
  width: 80%;
  margin: 0 auto;
  height: 100%;
  padding: 50px;
}

.social-icon-wrapper {
  display: flex;
  justify-content: center;
  align-items: center;
  width: 48px;
  height: 48px;
  border-radius: 50%;
  transition:
    background-color 0.3s ease,
    color 0.3s ease;
  color: #9ca3af;
}

.social-icon-wrapper:hover {
  background-color: #000000;
  color: #ffffff;
}

.social-icon-wrapper svg {
  fill: currentColor;
}

/* Responsive Design */
@media (max-width: 1024px) {
  .my-custom-team-container {
    padding: 50px 20px;
    height: auto;
  }

  .team-container {
    width: 90%;
    padding: 30px;
    height: auto;
  }

  .title {
    font-size: 2.5rem;
  }

  .card-embuh {
    margin-top: 60px;
    gap-y: 40px;
  }

  .text-embuh {
    font-size: 1.125rem;
    margin-top: 30px;
  }
}

@media (max-width: 768px) {
  .my-custom-team-container {
    padding: 30px 15px;
    height: auto;
  }

  .team-container {
    width: 95%;
    padding: 20px;
    height: auto;
  }

  .title {
    font-size: 2rem;
  }

  .card-embuh {
    margin-top: 40px;
    gap-y: 30px;
  }

  .text-embuh {
    font-size: 1rem;
    margin-top: 20px;
  }

  .logo-embuh {
    margin-top: 15px;
  }
}

@media (max-width: 640px) {
  .my-custom-team-container {
    padding: 20px 10px;
  }

  .team-container {
    width: 100%;
    padding: 15px;
  }

  .title {
    font-size: 1.75rem;
  }

  .text-embuh {
    font-size: 0.95rem;
  }

  .social-icon-wrapper {
    width: 40px;
    height: 40px;
  }

  .social-icon-wrapper svg {
    width: 32px;
    height: 32px;
  }
}
</style>
